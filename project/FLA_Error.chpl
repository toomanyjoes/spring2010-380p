...
void FLA_Error_messages_init( void )
{
	...
	sprintf( FLA_Error_string_for_code(FLA_UNEQUAL_VECTOR_LENGTHS),
             "Detected vectors of unequal length." );
	sprintf( FLA_Error_string_for_code(FLA_INVALID_VECTOR_LENGTH),
             "Detected vector of unexpected length." );
	...
	sprintf( FLA_Error_string_for_code(FLA_VECTOR_LENGTH_BELOW_MIN),
             "Detected vector length below pre-determined minimum." );
...
def FLA_Print_message( str: string, file: string, line: int )
{
	...
	writeln("libflame: ", file, " (line ", line, "):");	//fprintf( stderr, "libflame: %s (line %d):\n", file, line );
	writeln("libflame: ", str);	//fprintf( stderr, "libflame: %s\n", str );
	//fflush( stderr );
}
